...
- Compaction notice is now one unified `✓ Context compacted · {elapsed}` transcript card (before → after tokens, saved %, message counts, window share) for threshold/overflow/manual paths — same numbers, same shape; the manual summary follows as normal chat text instead of dim `└` output
